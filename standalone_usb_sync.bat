...
REM ==========================================
REM USB SYNC - STANDALONE (NO PARAMETERS!)
REM ==========================================
REM Just double-click to run!
REM Auto-finds config in same folder
REM ==========================================

title USB Sync - Standalone

REM ==========================================
REM AUTO-DETECT CONFIG AND LOG
REM ==========================================
...
for /f "tokens=1-3 delims=:." %%a in ('echo %time%') do set LOG_TIME=%%a%%b%%c
set LOG_TIME=%LOG_TIME: =0%
set LOGFILE=%LOG_DIR%\sync_%LOG_DATE%_%LOG_TIME%.log
...
if not exist "%CONFIG_FILE%" (
    ...
    echo.
    echo Please create sync_config.ini in:
    echo %SCRIPT_DIR%
    echo.
    pause
    exit /b 1
...
REM ==========================================
REM READ CONFIG (in original codepage)
REM ==========================================
...
REM Switch to UTF-8 (65001)
chcp 65001 >nul 2>&1

cls
echo ============================================
echo USB SYNC MODULE - STANDALONE
echo ============================================
echo.
...
REM Create log file
echo ======================================= > "%LOGFILE%"
echo USB SYNC - STANDALONE >> "%LOGFILE%"
echo ======================================= >> "%LOGFILE%"
echo Started: %date% %time% >> "%LOGFILE%"
echo Config: %CONFIG_FILE% >> "%LOGFILE%"
echo USB Drive: %USB_DRIVE% >> "%LOGFILE%"
echo ======================================= >> "%LOGFILE%"
echo. >> "%LOGFILE%"

REM Verify USB drive
if not defined USB_DRIVE (
    echo ERROR: USB drive not configured!
    echo.
    echo Please add this to sync_config.ini:
    echo [USB]
    echo drive=G:
    echo.
    pause
...
if not exist "%USB_DRIVE%\" (
    echo ============================================
    echo ERROR: USB drive %USB_DRIVE% not connected!
    echo ============================================
    echo.
...
echo [%time%] Skipped: %PHASE2_SKIPPED% >> "%LOGFILE%"
echo [%time%] --------------------------------------- >> "%LOGFILE%"
...
echo [%time%] ======================================= >> "%LOGFILE%"
echo [%time%] USB SYNC COMPLETE >> "%LOGFILE%"
echo [%time%] ======================================= >> "%LOGFILE%"
...
if errorlevel 8 (
    echo   ERROR: Mirror failed (code: !ERRORLEVEL!)
    echo [%time%] ERROR: Mirror failed for %KEY% (code: !ERRORLEVEL!) >> "%LOGFILE%"
    set /a PHASE1_FAILED+=1
) else (
    echo   SUCCESS (code: !ERRORLEVEL!)
